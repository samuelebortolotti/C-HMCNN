...
def main():
    """Main function for the training phase"""
    ...
    parser.add_argument(
        "--batch_size", type=int, required=True, help="input batch size for training"
    )
    ...
    parser.add_argument(
        "--hidden_dim", type=int, required=True, help="size of the hidden layers"
    )
    parser.add_argument(
    ...
    if "others" in args.dataset:
        ...
        train.X, valX, train.Y, valY = train_test_split(
            train.X, train.Y, test_size=0.30, random_state=seed
        )
    else:
    ...
    different_from_0 = torch.tensor(
        np.array((test.Y.sum(0) != 0), dtype=np.uint8), dtype=torch.uint8
    )
    ...
    if "others" in args.dataset:
        ...
        ).to(device), torch.tensor(train.Y).to(device)
        valX, valY = torch.tensor(scaler.transform(imp_mean.transform(valX))).to(
            device
        ), torch.tensor(valY).to(device)
    else:
    ...
    for epoch in range(num_epochs):
        ...
        train_score = average_precision_score(
            labels[:, train.to_eval],
            constr_output.data[:, train.to_eval],
            average="micro",
        )
        ...
        score = average_precision_score(
            y_val[:, train.to_eval], constr_val.data[:, train.to_eval], average="micro"
        )
        ...
        floss.write(
            "\nEpoch: {} - Loss: {:.4f}, Accuracy train: {:.5f}, Accuracy: {:.5f}, Precision score: ({:.5f})\n".format(
                epoch,
                loss,
                float(correct_train) / float(total_train),
