...
def main():
    """Main function of teh C-HMCNN model
    It aims to train and validate the neural network
    """

    # create the argument parser
    parser = argparse.ArgumentParser(
        description="Train neural network on train and validation set"
    )
    ...
    # required for the dataset names
    assert "_" in args.dataset
    assert "FUN" in args.dataset or "GO" in args.dataset or "others" in args.dataset

    # Load train, val and test set
    dataset_name = args.dataset
    data = dataset_name.split("_")[0]  # first part is the data
    ontology = dataset_name.split("_")[1]  # second part is the ontology
