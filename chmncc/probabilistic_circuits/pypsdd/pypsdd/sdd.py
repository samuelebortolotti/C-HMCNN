...
def logsumexp(tensor: Tensor, dim: int, keepdim: bool = False) -> Tensor:
    with torch.no_grad():
        ...
        m = m.masked_fill_(torch.isneginf(m), 0.0)
...
class SddNode:
    ...
    def __iter__(self, first_call=True, clear_data=False):
        ...
        if self.is_mixing():
            for e in elements:
                for node in e.__iter__(first_call=False):
                    yield node

        if self.is_decomposition():
            for p, s in self.elements:
                for node in p.__iter__(first_call=False):
                    yield node
                for node in s.__iter__(first_call=False):
                    yield node
        yield self
    ...
    def pre_order(self, first_call=True, clear_data=False):
        ...
        if self.is_mixing():
            for e in elements:
                for node in e.pre_order(first_call=False):
                    yield node

        if self.is_decomposition():
            for p, s in self.elements:
                for node in p.pre_order(first_call=False):
                    yield node
                for node in s.pre_order(first_call=False):
                    yield node
    ...
    def model_count(self, vtree):
        ...
        if self.is_false():
            return 0
        if self.is_true():
            return 1 << vtree.var_count
        if self.is_literal():
            return 1 << (vtree.var_count - 1)
    ...
    def is_model_marker_torch(self, inst, clear_bits=True, clear_data=True):
        ...
        if self.is_false():
            ...
            if (node.literal > 0 and inst[node.literal - 1]) or (
                node.literal < 0 and inst[-node.literal - 1]
            ):
                is_model = inst[self.vtree.var - 1]
            else:
                is_model = None
            ...
            for p, s in self.elements:
                if s.is_false_sdd:
                    continue  # for normalized SDDs
                if p.is_model_marker(inst, clear_bits=False) is None:
                    continue
                if s.is_model_marker(inst, clear_bits=False) is None:
                    continue
                is_model = (p, s)
                break
        self.data = is_model
    ...
    def models(self, vtree, lexical=False):
        ...
        iterator = (
            self._models_lexical(vtree) if lexical else self._models_recursive(vtree)
        )
        return iterator
    ...
    def minimum_cardinality(self):
        ...
        if self.is_true():
            return 0
        if self.is_literal():
            return 1 if self.literal > 0 else 0
...
class NormalizedSddNode(SddNode):
    ...
    def __init__(self, node_type, alpha, vtree, manager):
        ...
        if node_type is SddNode.DECOMPOSITION:
            self.positive_elements = tuple(
                (p, s) for p, s in self.elements if not s.is_false_sdd
            )
    ...
    def positive_iter(self, first_call=True, clear_data=False):
        ...
        if self.is_mixing():
            for e in self.elements:
                for node in e.positive_iter(first_call=False):
                    yield node
    ...
    def rand_parameters(self):
        ...
        for node in self.positive_iter():
            if node.is_literal():
                continue
            node.theta = torch.rand(
                966, len(node.positive_elements), device=DEVICE, requires_grad=True
            )
            node.theta = torch.nn.functional.softmax(
                node.theta, dim=-1
            )  # dict( (el,0.0) for el in root.positive_elements )
            # node.theta_sum = 0.0
    ...
    def get_mpe(self, batch_size, clear_data=True):
        ...
        for node in self.as_positive_list(clear_data=clear_data):
            ...
            if node.is_false():
                ...
                data = torch.where(
                    node.theta.argmax(dim=-2) > 0,
                    torch.tensor(node.vtree.var, device=DEVICE),
                    torch.tensor(-node.vtree.var, device=DEVICE),
                )
                data = data.unsqueeze(dim=-2)
                assert (
                    len(data.shape) == 3
                    and data.shape[0] == len(node.theta)
                    and data.shape[1] == 1
                )

            elif node.is_literal():
                data = torch.full(
                    (batch_size, 1, self.num_reps), node.literal, device=DEVICE
                )
                ...
                assert node.theta.shape[1] == batch_size and node.theta.shape[0] == len(
                    node.elements
                )
    ...
    def generate_normalized_ac(self, litleaves, clear_data=False):
        ...
        for node in self.as_list(clear_data=clear_data):
            ...
            if node.is_false():
                ...
                node.theta = [
                    litleaves[node.vtree.var - 1][0],
                    litleaves[node.vtree.var - 1][1],
                ]
    ...
    def generate_normalized_torch_ac_stable(self, litleaves, clear_data=False):
        ...
        for node in self.as_positive_list(clear_data=clear_data):
            ...
            if node.is_true():
                ...
                # We associate parameters for true literals, normalized
                # according to the corresponding vtree
                node.theta = torch.stack(
                    [litleaves[node.vtree.var - 1][0], litleaves[node.vtree.var - 1][1]]
                )
                ...
                node.theta = torch.stack(
                    [
                        p.data.clamp(min=1e-16).log() + s.data.clamp(min=1e-16).log()
                        for p, s in node.positive_elements
                    ]
                )
    ...
    def Shannon_entropy_stable(self, clear_data=False):
        ...
        for node in self.as_positive_list(clear_data=clear_data):
            ...
            if node.is_false():
                ...
                entropy = node.theta[0] * log(node.theta[0] + EPS) + node.theta[
                    1
                ] * log(node.theta[1] + EPS)
                ...
                for i, child_theta in enumerate(node.theta):
                    ...
                    entropy = entropy + (
                        child_theta * log(child_theta + EPS)
                        + child_theta * child_entropy
                    )
    ...
    def Shannon_entropy(self, litleaves, clear_data=False):
        for node in self.as_list(clear_data=clear_data):
            ...
            if node.is_false():
                ...
                # Entropy of a Bernoulli random variable
                entropy = litleaves[node.vtree.var - 1][0] * torch.log(
                    litleaves[node.vtree.var - 1][0] + EPS
                ) + litleaves[node.vtree.var - 1][1] * torch.log(
                    litleaves[node.vtree.var - 1][1] + EPS
                )
                ...
                for child in node.elements:
                    ...
                    child_theta = torch.div(
                        child[0].data * child[1].data, (node.data + EPS)
                    )
                    res.append(
                        child_theta * torch.log(child_theta + EPS)
                        + child_theta * child_entropy
                    )
                entropy = sum(res)
    ...
    def pr_inst(self, inst, clear_data=False):
        ...
        for node in self.as_list(clear_data=clear_data):
            ...
            if node.is_false():
                ...
                if node.literal > 0:
                    data = torch.tensor(
                        inst[node.literal - 1] > 0, device=DEVICE
                    )  # torch.cuda.current_device())
                else:
                    data = torch.tensor(
                        inst[-node.literal - 1] < 0, device=DEVICE
