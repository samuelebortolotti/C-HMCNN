...
class InstMap:
    ...
    def __setitem__(self, key, value):
        ...
        if value == 1 and (var not in self.inst or self.inst[var] is 0):
            self.bitset += 1 << (self.var_count - var)
        if (value == 0 or value is None) and (var in self.inst and self.inst[var] == 1):
            self.bitset -= 1 << (self.var_count - var)
    ...
    def __repr__(self, as_bitstring=True):
        if as_bitstring:
            st = {0: "0", 1: "1", None: "-"}
            inst = [
                self.inst[var] if var in self.inst else None
                for var in range(1, self.var_count + 1)
            ]
            return "".join(st[val] for val in inst)
        else:
            return " ".join(
                "%d:%d" % (var, self.inst[var]) for var in sorted(self.inst.keys())
            )
