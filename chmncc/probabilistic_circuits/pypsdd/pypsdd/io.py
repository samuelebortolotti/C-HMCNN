...
_dot_box_fmt = (
    "\nn%ue%u:%s:c->n%u " "[arrowsize=.50,tailclip=false,arrowtail=dot,dir=both];"
)
...
def _node_label(root, labels=_dot_names):
    ...
    if root.is_false():
        ...
        return "&#8868;"
    elif root.is_literal():
        return _literal_label(root.literal, labels=labels)
    else:
        return "   "
...
def _psdd_node_label(root, subcircuit=None, labels=_dot_names):
    ...
    if root.is_true():
        ...
        return _node_label(root, labels=labels)
...
def _psdd_jason_repr(self, use_index=False):
    ...
    if self.is_false():  # no FALSE nodes
        ...
        els = self.elements
        st_el = " ".join(
            "%d %d %.6f" % (index(p), index(s), log(self.theta[(p, s)])) for p, s in els
        )
